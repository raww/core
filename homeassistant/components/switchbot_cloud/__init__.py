...
def make_device_data(
    ...
    for device in devices:
        ...
        if isinstance(device, Device) and device.device_type == "WoSweeperMini":
...
async def async_setup_entry(hass: HomeAssistant, config: ConfigEntry) -> bool:
    ...
    webhook_id = config.data.get(CONF_WEBHOOK_ID)
    if webhook_id:
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    """Unload a swtichbot_cloud config entry."""
...
async def async_remove_entry(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Remove a switchbot_cloud entry."""
    ...
    cloudhook_url = hass.data["cloud"].client.cloudhooks[webhook_id]["cloudhook_url"]
    await api.delete_webhook(cloudhook_url)
